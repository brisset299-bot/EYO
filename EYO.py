...
def simular_semana_arena(
    ...
    """
    Simulación agregada equivalente a la primera lógica planteada
    para Arena:

        Llegadas -> cola FCFS -> capacidad médica agregada
        -> atención -> salida.

    Se utiliza un pool agregado porque el caso entrega horas-médico
    y una tasa promedio, no horarios individuales.

    Cada día se simula de forma independiente; la cola no se
    arrastra al siguiente día.

    El mix Sénior/Júnior no recibe tasas diferentes porque el caso
    no proporciona tasas separadas. Su efecto se representa
    únicamente mediante la ganancia de eficiencia configurable.
    """
    resultados = []

    pacientes = np.asarray(pacientes, dtype=float)
    horas_medico = np.asarray(horas_medico, dtype=float)
    ...
    for rep in range(int(replicas)):
        ...
        for d, dia in enumerate(DIAS):
            ...
            tasa = float(tasa_base)
            if d == 0:
                tasa *= 1.0 + float(ganancia_domingo) / 100.0
            ...
            for t_llegada in llegadas:
                servicio = rng.exponential(
                    scale=servicio_medio_min
                )
                ...
                # Aproximación de la cola para visualización.
                if espera > 0:
                    cola_actual = max(
                        1,
                        int(np.ceil(
                            espera / max(servicio_medio_min, 1e-12)
                        )),
                    )
            ...
            utilizacion = (
                n / capacidad_dia
                if capacidad_dia > 0 else np.nan
            )
            ...
            resultados.append({
                ...
                "Espera promedio (min)": (
                    np.mean(esperas) if esperas else np.nan
                ),
                "Espera P90 (min)": (
                    np.percentile(esperas, 90)
                    if esperas else np.nan
                ),
                "Tiempo sistema promedio (min)": (
                    np.mean(tiempos_sistema)
                    if tiempos_sistema else np.nan
                ),
...
with st.sidebar:
    ...
    if escenario.startswith("Escenario 1"):
        ...
        tasa_2 = st.number_input(
            "Tasa promedio de servicio (pac/h-médico)",
            min_value=0.1,
            max_value=10.0,
            value=2.40,
            step=0.05,
        ...
        senior_dom = st.slider(
            ...
            value=84,
            step=1,
            help="Se modifica el mix manteniendo constantes las 264 horas-médico del domingo.",
        )
        ...
        mejora_dom = st.slider(
            ...
            value=5.0,
            step=0.5,
            help="Supuesto del modelo. El caso entrega una tasa promedio de 2.40 pac/h-médico, no tasas separadas por tipo de médico.",
        )
        ...
        modo_llegadas_2 = st.radio(
            ...
            ["Estable", "Variable"],
            index=0,
            help="Estable: llegadas uniformes. Variable: interarribos exponenciales normalizados para conservar la demanda diaria.",
        )
...
if escenario.startswith("Escenario 1"):
    ...
    senior_actual = 60.0
    junior_actual = 204.0
    horas_total_domingo = 264.0

    senior_nuevo = float(senior_dom)
    junior_nuevo = horas_total_domingo - senior_nuevo

    mix_actual = senior_actual / horas_total_domingo
    mix_nuevo = senior_nuevo / horas_total_domingo

    # El caso entrega una única tasa promedio de 2.40
    # pac/hora-médico. El cambio de mix no altera por sí
    # mismo la tasa; el efecto adicional se parametriza.
    tasa_efectiva_domingo = tasa_2 * (
        1.0 + mejora_dom / 100.0
    )

    tasa_esc2 = np.full(7, tasa_2, dtype=float)
    tasa_esc2[0] = tasa_efectiva_domingo
    ...
    capacidad_actual = H_BASE * tasa_2
    capacidad_esc2 = H_BASE * tasa_esc2

    util_actual = DEMANDA / capacidad_actual
    util_esc2 = DEMANDA / capacidad_esc2

    lam_2 = DEMANDA / horas_dia_2
    mu_actual_2 = capacidad_actual / horas_dia_2
    ...
    wq_actual[estable_actual] = (
        ...
        / (
            mu_actual_2[estable_actual]
            * (mu_actual_2[estable_actual] - lam_2[estable_actual])
        )
    )
    ...
    wq_esc2[estable_esc2] = (
        ...
        / (
            mu_esc2_2[estable_esc2]
            * (mu_esc2_2[estable_esc2] - lam_2[estable_esc2])
        )
    )
    ...
    tabla_2 = pd.DataFrame({
        ...
        "Horas médico": H_BASE,
        "Pacientes": DEMANDA,
        "Tasa servicio": tasa_esc2,
        "Capacidad": capacidad_esc2,
    ...
    d.metric(
        ...
        f"{tasa_2:.2f} → {tasa_efectiva_domingo:.2f}",
    )

    st.info(
        f"Se mantienen las 264 horas-médico del domingo "
        f"({senior_nuevo:.0f} h Sénior + {junior_nuevo:.0f} h Júnior). "
        f"El caso no proporciona tasas distintas por tipo de médico; "
        f"por ello, el efecto del nuevo mix se representa mediante "
        f"una ganancia de eficiencia supuesta de {mejora_dom:.1f}%."
    )
    ...
    ax.bar(
        ["Actual", "Escenario 2"],
        [util_actual[0], util_esc2[0]],
    )
    ax.set_ylabel("Utilización")
    ax.set_title("Utilización del domingo")
    ax.set_ylim(
        0,
        max(1.0, float(max(util_actual[0], util_esc2[0])) * 1.15),
    )
    ...
    st.caption(
        "Primera lógica de simulación: llegadas → cola FCFS → "
        "pool agregado de capacidad médica → atención → salida. "
        "La demanda diaria se mantiene exactamente y cada día "
        "se simula de manera independiente."
    )
    ...
    if st.button(
        ...
        with st.spinner(
            f"Ejecutando {replicas_2} réplicas..."
        ):
    ...
    if (
        ...
        reduccion = (
            (1.0 - espera_b / espera_a) * 100.0
            if espera_a > 0 else 0.0
        )
        ...
        comp = pd.DataFrame({
            ...
            "Actual": [
                espera_a,
                ga["P90 promedio diario (min)"].mean(),
                ga["Cola máxima semanal"].mean(),
                ga["Pacientes pendientes"].mean(),
                0.0,
            ],
            "Escenario 2": [
                espera_b,
                gb["P90 promedio diario (min)"].mean(),
                gb["Cola máxima semanal"].mean(),
                gb["Pacientes pendientes"].mean(),
                reduccion,
            ],
        ...
        ax.set_title(
            "Tiempo promedio de espera — Actual vs Escenario 2"
        )
        ax.legend()
        ax.grid(axis="y", alpha=0.25)
        st.pyplot(fig, use_container_width=True)
        ...
        st.success(
            f"Bajo el supuesto de {mejora_dom:.1f}% de ganancia de "
            f"eficiencia, la espera promedio semanal simulada pasa "
            f"de {espera_a:.2f} a {espera_b:.2f} minutos. "
            f"El total del domingo permanece en 264 horas-médico."
        )

    st.download_button(
        "⬇️ Descargar resultados Escenario 2",
        data=excel_bytes({
            "Escenario2_Capacidad": tabla_2,
        }),
